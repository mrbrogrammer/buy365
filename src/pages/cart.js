...
const Cart = ({location}) => {
  ...
  async function getCartItems() {
    ...
    await Shopify.getCartItems(cartIdLocal).then(({data, meta}) => {
      setItems(data)
  ...
  const handleCheckout = async data => {
    ...
    try {
      const {
        data: {id},
      } = await Shopify.checkoutCart(cartId, customer, address)
      await Shopify.payForOrder(id, token, email)
      setCompleted(true)
  ...
  const handleRemoveFromCart = itemId => {
    Shopify.removeFromCart(itemId, cartId).then(({data, meta}) => {
